...
class QuestionsClient(BaseAPI):
    ...
    def create(self, category_id: int, question_text: str, answer_text: str) -> APIResponse:
        ...
        return self.http.post(ConfigTests.QUESTIONS_ENDPOINT, json=payload)
    ...
    def get_all(self, category_id: int | None = None) -> APIResponse:
        ...
        return self.http.get(ConfigTests.QUESTIONS_ENDPOINT, params=params)

    def get_by_id(self, question_id: int) -> APIResponse:
        """Sends a request to retrieve a question by its ID."""
        return self.http.get(f"{ConfigTests.QUESTIONS_ENDPOINT}/{question_id}")
    ...
    def update(
        ...
        return self.http.put(f"{ConfigTests.QUESTIONS_ENDPOINT}/{question_id}", json=payload)

    def delete(self, question_id: int) -> APIResponse:
        """Sends a request to delete a question by its ID."""
        return self.http.delete(f"{ConfigTests.QUESTIONS_ENDPOINT}/{question_id}")
